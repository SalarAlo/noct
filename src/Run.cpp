...
RunResult RunFromString(Context& context, std::string_view contents) {
	...
	return RunFromString(context, contents, modules);
}

RunResult RunFromString(Context& context, std::string_view contents, const ModuleRegistry& modules) {
	...
	Interpreter interpreter { context, modules };
