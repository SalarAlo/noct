...
void Resolver::EndScope() {
	...
	for (const auto& [name, md] : scope.Slots) {
		...
		case SymbolKind::Module:
			if (md.ReadCount == 0)
				Logger::Warn("Module {} is unused", name);
			break;
		}
	}
...
void Resolver::operator()(ImportStatement& import) {
	const auto slot = TryDeclareInCurrentScope(import.Name, SymbolKind::Module);
	if (!slot)
		return;

	import.Slot = *slot;
	GetCurrentScope().Slots[import.Name.Lexeme].Defined = true;
}
